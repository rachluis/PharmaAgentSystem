...
def fix_database():
    ...
    for cluster_id in cluster_ids:
        ...
        print(f"  [OK] Created cluster {cluster_id}: {cluster_name} ({size:,} doctors, {size_percentage:.1f}%)")
    ...
    print("  [OK] Upgraded 'Rui_3' to admin role")

    # Make Rui an analyst
    cursor.execute("UPDATE users SET role = 'analyst' WHERE username = 'Rui'")
    print("  [OK] Upgraded 'Rui' to analyst role")
    ...
    try:
        ...
        print("  [OK] Dropped unused 'system_logs' table")
        conn.commit()
    except Exception as e:
        print(f"  [WARNING] Could not drop system_logs: {e}")
    ...
    for cluster_id, cluster_name in cluster_names.items():
        ...
        if affected > 0:
            print(f"  [OK] Updated {affected:,} doctors in Cluster {cluster_id}")
    ...
    print(f"[PASS] cluster_results records: {cluster_count}")

    cursor.execute("SELECT role, COUNT(*) FROM users GROUP BY role")
    roles = cursor.fetchall()
    print(f"[PASS] User roles:")
    ...
    print(f"[PASS] system_logs table exists: {bool(system_logs_exists)}")

    cursor.execute("SELECT COUNT(*) FROM doctors WHERE cluster_label IS NOT NULL")
    labeled_count = cursor.fetchone()[0]
    print(f"[PASS] Doctors with cluster labels: {labeled_count:,}")
    ...
    print("=" * 80)
    cursor.execute("""
        SELECT cluster_id, cluster_name, size_count, size_percentage, kpi_summary
        FROM cluster_results
        ORDER BY cluster_id
    """)
    clusters = cursor.fetchall()
    for cluster in clusters:
        kpi = json.loads(cluster[4])
        print(f"\nCluster {cluster[0]}: {cluster[1]}")
        print(f"  Size: {cluster[2]:,} doctors ({cluster[3]:.1f}%)")
        print(f"  Avg Monetary: ${kpi['avg_monetary']:,.2f}")
        print(f"  Avg Frequency: {kpi['avg_frequency']:.2f}")
        print(f"  Top Specialty: {kpi['top_specialty']}")

    conn.close()

    print("\n" + "=" * 80)
    print("FIX COMPLETE - Database is now consistent!")
    print("=" * 80)
